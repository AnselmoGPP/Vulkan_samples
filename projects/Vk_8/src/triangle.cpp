...
bool HelloTriangleApp::checkValidationLayerSupport(const std::vector<const char*>& requiredLayers)
{
...
bool HelloTriangleApp::checkExtensionSupport(const char* const* requiredExtensions, uint32_t reqExtCount)
{
...
VkSampleCountFlagBits HelloTriangleApp::getMaxUsableSampleCount(bool getMinimum)
{
...
void HelloTriangleApp::createGraphicsPipeline()
{
	...
	multisampling.sampleShadingEnable	= (add_SS ? VK_TRUE : VK_FALSE);	// Enable sample shading in the pipeline
	if (add_SS)
...
void HelloTriangleApp::createTextureImage()
{
	...
	transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);					// Transition the texture image to VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
...
void HelloTriangleApp::createTextureSampler()
{
	...
	samplerInfo.mipmapMode				= VK_SAMPLER_MIPMAP_MODE_LINEAR;	// VK_SAMPLER_MIPMAP_MODE_ ... NEAREST (lod selects the mip level to sample from), LINEAR (lod selectS two mip levels to be sampled, and the results are linearly blended)
...
*/
void HelloTriangleApp::loadModel(const char* obj_file)
{
