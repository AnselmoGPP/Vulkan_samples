 ...
 *
	...
	Objects far away from the camera will sample their textures from the smaller mip images. This increases rendering speed and avoids artifacts (like Moiré patterns).
	...
	This is one way to fix the aliasing (jagged saw like patterns along the edges of drawn geometric shapes). In ordinary rendering, the pixel color is determined based
	on a single sample point in the center of the target pixel (usually). MSAA uses multile sample points per pixel to determine its final color.
	In MSAA, each pixel is sampled in an offscreen buffer which is then rendered to the screen. This new buffer has to be able to store more than one sample per pixel. 
...
private:
	...
	VkSampleCountFlagBits HelloTriangleApp::getMaxUsableSampleCount(bool minimum = false);
