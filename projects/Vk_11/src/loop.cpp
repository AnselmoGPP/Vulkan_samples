...
loopManager::loopManager(std::vector<modelConfig>& modelConfigs)
	: input(e.window)
{ 
	// Get the models data
...
void loopManager::updateUniformBuffer(uint32_t currentImage)
{
	...
	uboTemp.view = input.cam.GetViewMatrix();
	uboTemp.proj = input.cam.GetProjectionMatrix(e.swapChainExtent.width / (float)e.swapChainExtent.height);
