...
std::array<VkVertexInputAttributeDescription, 2> Vertex::getAttributeDescriptions()
{
	...
	attributeDescriptions[0].format		= VK_FORMAT_R32G32_SFLOAT;	// Type of data for the attribute: VK_FORMAT_ ... R32_SFLOAT (float), R32G32_SFLOAT (vec2), R32G32B32_SFLOAT (vec3), R32G32B32A32_SFLOAT (vec4), R64_SFLOAT (64-bit double), R32G32B32A32_UINT (uvec4: 32-bit unsigned int), R32G32_SINT (ivec2: 32-bit signed int)...
...
void HelloTriangleApp::createDescriptorSets() 
{
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) 
	{
		...
		descriptorWrite.dstSet			 = descriptorSets[i];						// Descriptor set to update
		descriptorWrite.dstBinding		 = 0;										// Binding
		descriptorWrite.dstArrayElement  = 0;										// First index in the array (if you want to update multiple descriptors at once in an array)
		descriptorWrite.descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;		// Type of descriptor
		descriptorWrite.descriptorCount  = 1;										// Number of array elements to update
		descriptorWrite.pBufferInfo		 = &bufferInfo;								// Used for descriptors that refer to buffer data (like our descriptor)
		...
		vkUpdateDescriptorSets(device, 1, &descriptorWrite, 0, nullptr);			// Accepts 2 kinds of arrays as parameters: VkWriteDescriptorSet, VkCopyDescriptorSet.
	}
